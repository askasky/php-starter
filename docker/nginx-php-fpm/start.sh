  ...
  if [ -f $XdebugFile ]; then
    ...
    if [ -f $XdebugFile ]; then
        ...
        if grep -q xdebug.remote_enable "$XdebugFile"; then
            ...
            echo "xdebug.remote_log=/var/log/xdebug.log"  >> $XdebugFile
...
while [ "$connect" != "0" ]; do
    echo "$connect"
    sleep 2
    connect=`php -r "mysqli_connect('mariadb','root','123456');" &>/dev/stdout| wc -l`
done
...
echo | php artisan passport:client --password
